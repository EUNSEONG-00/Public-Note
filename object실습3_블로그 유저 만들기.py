...
class BlogUser:
    ...
    def __init__(self, name):
        ...
        posts는 빈 배열로 초기화한다 -> 비어있는 리스트로 설정
        """
        self.name = name
        self.posts = []
    ...
    def show_all_posts(self):
        # 블로그 유저의 모든 게시글 출력
        # 블로그 유저의 게시글들을 모두 출력하는 show_all_posts 메소드 생성
        # 인스턴스를 출력하려면 print() 함수를 사용하면 되는데요.
        # print() 함수에 인스턴스를 넣으면 그 인스턴스의 __str__() 메소드가 실행됨.
        # 게시글 인스턴스를 출력하면 Post 클래스의 __str__() 메소드가 리턴하는 값인 게시일과 내용이 출력될 겁니다.
