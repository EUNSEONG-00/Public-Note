...
# 그 전에 일단 '시계'라는 객체도 클래스로 정의해야겠죠? 이전의 레슨에서 보았듯이, 만들려는 시계 프로그램은 다음 조건들을 만족해야 합니다.
#
# 현재 시간을 설정할 수 있다.
# 현재 시간을 변경할 수 있다.
# 현재 시간에 1초씩 더할 수 있다.
# Counter 클래스를 조합하면 시계의 기능을 완성할 수 있는데요.
# 시계를 나타내는 클래스의 이름은 Clock으로 하겠습니다.
# Clock 클래스가 가질 속성과 행동을 먼저 봅시다.
#
# 속성
...
# 이것은 당연한 시간의 원리이니 따로 설명하지 않겠습니다. 이 부분을 구현할 때 Counter 클래스의 tick 메소드의 리턴값(True 또는 False)이 어떻게 활용될지 생각해 보세요.
...
class Clock:
    ...
    def __init__(self, hour, minute, second):
        ...
        Counter 클래스의 set 메소드로 현재의 시,분,초를 설정
        """
